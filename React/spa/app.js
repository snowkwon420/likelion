...
async function newsFeed() {
  ...
  const newsList = [];
  newsList.push(
    '<ul class="bg-gray-500 flex flex-col gap-4 h-5/6 items-center justify-center">'
  ); // ul 시작 태그를 담아둡니다.
  ...
  newsList.push(
    `<nav class="bg-gray-500 flex flex-col gap-4 h-1/6 items-center justify-center">
            ...
            }" class="relative block rounded bg-transparent px-3 py-1.5 text-sm text-neutral-600 transition-all duration-300 hover:bg-neutral-100 dark:text-white dark:hover:bg-neutral-700 dark:hover:text-white">이전</a></li>
            ${pageList}
            <li><a href="#/page/${
              store.currentPage >= newsFeed.length / store.datasPerPage
                ? store.currentPage
                : store.currentPage + 1
            }" class="relative block rounded bg-transparent px-3 py-1.5 text-sm text-neutral-600 transition-all duration-300 hover:bg-neutral-100 dark:text-white dark:hover:bg-neutral-700 dark:hover:text-white">다음</a></li>
            </ul>
        </nav>`
  );
